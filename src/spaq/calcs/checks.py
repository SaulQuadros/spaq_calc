...
def pressure_budget_messages(points_df: pd.DataFrame, model_row, q_per_unit_lpm: float, supply_dyn_kpa: float, mixer_dp_kpa: float = 20.0):
    ...
    Não inclui perdas de rede (tubulação) — pode ser somado externamente se informado.
    """
    msgs = []
    try:
        # montar curva dp do catálogo se disponível
        dp_curve = {}
        for col in ["dp_lpm_8_kpa","dp_lpm_10_kpa","dp_lpm_12_kpa"]:
            if col in model_row and not pd.isna(model_row[col]):
                lpm = int(col.split("_")[2])  # 8, 10, 12
                dp_curve[float(lpm)] = float(model_row[col])
